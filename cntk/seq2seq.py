...
embedding_dim = 200
vocab = [
    w.strip() for w in open(os.path.join(DATA_DIR, VOCAB_FILE)).readlines()
]  # all lines of VOCAB_FILE in a list
length_increase = 1.5
...
model_path_stem = os.path.join(
    MODEL_DIR, "model_att_{}".format(use_attention)
)  # encode as many config vars as desired
...
def create_model():  # :: (history*, input*) -> logP(w)*
    ...
    with default_options(
        enable_self_stabilization=True, go_backwards=not use_attention
    ):
        ...
        encode = Sequential(
            [
                embed,
                Stabilizer(),
                For(range(num_layers - 1), lambda: Recurrence(LSTM(hidden_dim))),
    ...
    with default_options(enable_self_stabilization=True):
        ...
        if (
            use_attention
        ):  # maps a decoder hidden state and the entire encoder state into an augmented decoder state
            attention_model = AttentionModel(
                attention_dim, attention_span, attention_axis, name="attention_model"
                ...
                if use_attention:
                    if i == 0:
                        ...
                        def lstm_with_attention(dh, dc, x):
                            ...
                            x = splice(
                                x, h_att
                            )  # TODO: should this be added instead? (cf. BS example)
                    ...
                    r = RecurrenceFrom(rec_block)(
                        *(encoded_input.outputs + (r,))
                    )  # :: h0, c0, r -> h  (Python < 3.5)
...
def create_model_greedy(s2smodel):
    ...
    def model_greedy(input):  # (input*) --> (word_sequence*)
        ...
        unfold = UnfoldFrom(
            lambda history: s2smodel(history, input) >> hardmax,
            until_predicate=lambda w: w[
                ..., sentence_end_index
...
def create_criterion_function(model):
    @Function
    @Signature(
        input=InputSequence[Tensor[input_vocab_dim]],
        labels=LabelSequence[Tensor[label_vocab_dim]],
    )
    def criterion(input, labels):
        # criterion function must drop the <s> from the labels
        postprocessed_labels = sequence.slice(
            labels, 1, 0
        )  # <s> A B C </s> --> A B C </s>
...
def evaluate_metric(reader, s2smodel, num_minibatches=None):
    ...
    while True:
        ...
        mb_error = evaluator.test_minibatch(
            {
                criterion.arguments[0]: mb[reader.streams.features],
...
def translate(
    tokens, model_decoding, vocab, i2w, show_attention=False, max_label_length=20
):
    ...
    if use_attention and show_attention:
        ...
        att_value = att_value[
            0, 0 : len(prediction), 0 : len(w), 0, 0
        ]  # -> (len, span)
...
def interactive_session(s2smodel, vocab, i2w, show_attention=False):
    ...
    while True:
        ...
        for word in line.split():
            ...
            out_tokens = translate(
                in_tokens, model_decoding, vocab, i2w, show_attention=True
            )
...
if __name__ == "__main__":
    ...
    train(
        train_reader, valid_reader, vocab, i2w, model, max_epochs=30, epoch_size=908241
    )
